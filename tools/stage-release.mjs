...
const browserUploadRoot = path.join(releaseRoot, "github-upload-v0.8.2");
const browserDotfilesRoot = path.join(releaseRoot, "github-dotfiles-upload-v0.8.2");
const manifest = JSON.parse(await fs.readFile(path.join(moduleRoot, "module.json"), "utf8"));

if (manifest.version !== "0.8.2") {
  throw new Error(`Expected module version 0.8.2 but found ${manifest.version}.`);
}
...
const browserReleaseFiles = [
  ...
  "module.json",
  "package.json",
  "scripts/cwn-interface-theme-v082.mjs",
  "scripts/sheets/cwn-character-sheet-v082.mjs",
  "styles/cwn-interface-theme-v082.css",
  "templates/sheets/character/skills-v082.hbs",
