...
const browserUploadRoot = path.join(releaseRoot, "github-upload-v0.5.2");
const browserDotfilesRoot = path.join(releaseRoot, "github-dotfiles-upload-v0.5.2");
const manifest = JSON.parse(await fs.readFile(path.join(moduleRoot, "module.json"), "utf8"));

if (manifest.version !== "0.5.2") {
  throw new Error(`Expected module version 0.5.2 but found ${manifest.version}.`);
}
