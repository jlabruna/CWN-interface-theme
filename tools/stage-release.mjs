...
const browserUploadRoot = path.join(releaseRoot, "github-upload-v0.6.3");
const browserDotfilesRoot = path.join(releaseRoot, "github-dotfiles-upload-v0.6.3");
const manifest = JSON.parse(await fs.readFile(path.join(moduleRoot, "module.json"), "utf8"));

if (manifest.version !== "0.6.3") {
  throw new Error(`Expected module version 0.6.3 but found ${manifest.version}.`);
}
