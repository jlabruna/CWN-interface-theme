...
} from "./cwn-sheet-shared-v061.mjs";
...
export function createCwnCharacterSheetClass(SWNActorSheet) {
  return class CwnCharacterSheet extends SWNActorSheet {
    ...
    static PARTS = {
      header: { template: `modules/${MODULE_ID}/templates/sheets/character/header-v061.hbs` },
      combat: { template: `modules/${MODULE_ID}/templates/sheets/character/combat-v061.hbs` },
      skills: { template: `modules/${MODULE_ID}/templates/sheets/character/skills-v061.hbs` },
      inventory: { template: `modules/${MODULE_ID}/templates/sheets/character/inventory-v061.hbs` },
      cyberware: { template: `modules/${MODULE_ID}/templates/sheets/character/cyberware-v061.hbs` },
      features: { template: `modules/${MODULE_ID}/templates/sheets/character/features-v061.hbs` },
      actions: { template: `modules/${MODULE_ID}/templates/sheets/character/actions-v061.hbs` },
      biography: { template: `modules/${MODULE_ID}/templates/sheets/character/biography-v061.hbs` },
    ...
    static async _onOpenActionsTab(event) {
      event.preventDefault();
      if (typeof this.changeTab === "function") {
        await this.changeTab("actions", "primary", { event });
        return;
      }
      this.tabGroups.primary = "actions";
      await this.render({ parts: ["actions", "tabs"] });
    }
