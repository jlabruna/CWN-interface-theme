...
function cargoAmmo(item) {
  const system = item?.system ?? {};
  const uses = system.uses;
  const ammo = item?.type === "item" && uses?.ammo && uses.ammo !== "none"
    ? { type: uses.ammo, value: uses.value, max: uses.max }
    : system.ammo;
  if (!ammo?.type || ammo.type === "none" || ammo.type === "infinite") return null;
  const value = number(ammo.value);
  const maximum = number(ammo.max);
  return {
    empty: value <= 0,
    label: value <= 0 ? `0/${maximum} · Empty` : `${value}/${maximum}`,
  };
}
...
function fittingCost(item) {
  if (item?.type !== "shipFitting" && item?.type !== "shipDefense") return 0;
  return Math.max(0, number(item?.system?.mass, 1));
}

export function checkFittingCapacity(actor, itemData = [], user = globalThis.game?.user) {
  const additions = Array.isArray(itemData) ? itemData : [itemData];
  const added = additions.reduce((total, item) => total + fittingCost(item), 0);
  const remaining = number(actor?.system?.fittings?.value, actor?.system?.fittings?.max);
  const exceeded = added > 0 && added > remaining;
  return {
    added,
    remaining,
    projected: remaining - added,
    exceeded,
    canOverride: exceeded && Boolean(user?.isGM),
  };
}

export function isDroneDeployed(actor) {
  return Boolean(
    actor?.getFlag?.(MODULE_ID, DRONE_DEPLOYED_FLAG)
      ?? actor?.flags?.[MODULE_ID]?.[DRONE_DEPLOYED_FLAG],
  );
}
...
async function setDroneDeployed(actor, deployed) {
  if (typeof actor?.setFlag === "function") {
    await actor.setFlag(MODULE_ID, DRONE_DEPLOYED_FLAG, deployed);
    return;
  }
  await actor?.update?.({ [`flags.${MODULE_ID}.${DRONE_DEPLOYED_FLAG}`]: deployed });
}

async function approveFittingCapacity(actor, itemData) {
  const check = checkFittingCapacity(actor, itemData);
  if (!check.exceeded) return true;

  const message = `${actor.name} has ${check.remaining} fitting capacity remaining; this would use ${check.added} more.`;
  if (!check.canOverride) {
    globalThis.ui?.notifications?.warn?.(`${message} Ask the GM to approve an over-capacity custom drone.`);
    return false;
  }

  const confirm = globalThis.foundry?.applications?.api?.DialogV2?.confirm;
...
export function createCwnDroneSheetClass(SWNVehicleSheet) {
  return class CwnDroneSheet extends SWNVehicleSheet {
    ...
    static PARTS = {
      ...
      operations: { template: `modules/${MODULE_ID}/templates/sheets/drone/operations-v092.hbs` },
      fittings: { template: `modules/${MODULE_ID}/templates/sheets/drone/fittings-v092.hbs` },
      cargo: { template: `modules/${MODULE_ID}/templates/sheets/drone/cargo-v092.hbs` },
    ...
    async _onDropItemCreate(itemData, event) {
      if (!await approveFittingCapacity(this.actor, itemData)) return false;
    ...
    static async _onCreateDroneDocument(event, target) {
      if (target.dataset.type === "shipFitting") {
        ...
        if (!await approveFittingCapacity(this.actor, proposed)) return;
      }
      const nativeCreate = SWNVehicleSheet.DEFAULT_OPTIONS?.actions?.createDoc;
      if (typeof nativeCreate !== "function") {
        globalThis.ui?.notifications?.error?.("SWNR's native Item creation action is unavailable.");
        return;
      }
      await nativeCreate.call(this, event, target);
    }
    ...
    static async _onDeclareDroneAction(event, target) {
      ...
      if (target.dataset.actionKey === "deploy") {
        const deployed = !cwnit.deployed;
        await setDroneDeployed(this.actor, deployed);
        await postDroneChat(this.actor, deployed ? {
          ...
          note: "The drone is marked deployed. No Token was placed; use Assume Command when active control is required.",
        } : {
          title: `Pack ${this.actor.name}`,
          cost: "Main Action",
          details: [{ label: "Pilot", value: pilot }],
          note: "The drone is marked packed. No Token was removed, and its pilot link was not changed.",
        });
        return;
      }
      const actions = {
        ...
        drop: {
          ...
          cost: "On Turn",
          details: [{ label: "Pilot", value: pilot }],
          note: "Active control released. The drone remains deployed but inert until command is assumed again; its pilot link and deployment state are unchanged.",
        },
        halt: {
